...
export type RecipesStorageInterface = {
  ...
  addRecipe: (
    recipe: Recipe,
    onAvalible?: (id: string, recipe: Recipe) => void,
  ) => void;
  editRecipe: (
    editedRecipe: Recipe,
    id: string,
...
export default function useRecipeStorage(db: Firestore, userId: string | null) {
  ...
  const pendingWrites = useRef(
    new Map<string, (id: string, recipe: Recipe) => void>(),
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, `users/${userId}/recipies`), {
      ...
      const recipes: Map<string, Recipe> = new Map(
        querySnapshot.docs.map((doc) => {
          ...
          console.assert(
            ...
            `Recipe ${doc.id} pulled from DB has no name field`,
          );
      ...
      changeData.forEach((change) => {
        const writeCallback = pendingWrites.current.get(
      ...
      const source = querySnapshot.metadata.fromCache
        ? "local cache"
        : "server";
      console.log(`Recipes snapshot (from ${source}):`, {
        recipes: recipes,
        changes: changeData,
      });
      setRecipes({
