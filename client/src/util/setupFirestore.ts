  ...
  enableIndexedDbPersistence(db)
    .catch((err) => {
      if (err.code === "failed-precondition") {
        console.error(
          "FireStore IndexedDB - failed precondition: Multiple tabs open, persistence can only be enabled in one tab at a a time.",
        );
      } else if (err.code === "unimplemented") {
        console.error(
          "FireStore IndexedDB: The current browser does not support all of the features required to enable persistence",
        );
      }
    });
  const auth = getAuth();
