...
function IngredientsField({ setValue, control, register, trigger }: Props) {
  ...
  const setPercentageBasedOffIngredient = (
    subListIdx: number,
    localIdx: number,
    anchorIdx: number,
  ): void => {
    ...
    const percentage: number | undefined = getPercentageFromVal(
      ingredient,
      anchor,
    );
    if (percentage) {
  ...
  const setAllPercentages = (anchorIdx: number) => {
    for (let subListIdx = 0; subListIdx < lists.length; subListIdx++) {
      ...
      for (
        let localIdx = 0;
        localIdx < subList.ingredients.length;
        localIdx++
      ) {
  ...
  const onQuantityBlur =
    (subListIdx: number, localIdx: number) => (ev: any) => {
  ...
  const onPercentageBlur =
    (subListIdx: number, localIdx: number) => (ev: any) => {
      ...
      const anchorQuantity: IngredientQuantity = parseIngredientQuantity(
      ...
      if (isMathable) {
        ...
        const subjectUnit = typeof subjectQuantity !== "number" &&
            typeof subjectQuantity !== "string"
          ? subjectQuantity.unit
          : undefined;

        const newQuantity = getQuantityFromPercentage(
          anchorQuantity,
          subjectPercentage,
          subjectUnit,
        );
  ...
  const handleMultipleListsChange = (_hasMultipleLists: boolean) => {
    if (!_hasMultipleLists) {
      const confirmation = () =>
        window.confirm(
          "Are you sure you want to switch back to having a single list? This will remove all your list names and cannot be undone.",
        );
      if (!confirmation()) {
        return;
      }
    ...
    <>
      <div className="ingredientsHeader">
        ...
        <DropdownMenu
          trigger={
            <span>
              <MdMoreVert className="ingredientsHeaderMenu icon-button" />
            </span>
          }
          position={"left top"}
          offset={["-0.8rem", "0rem"]}
        >
          <MenuItemToggleable
            text="Use baker's percentages"
            value={isPercentagesIncluded}
            toggle={(b) => setIsPercentagesIncluded(b)}
          />
          <MenuItemToggleable
            text="Use multiple lists"
            value={hasMultipleLists}
            toggle={(b) => handleMultipleListsChange(b)}
          />
        </DropdownMenu>
      </div>
      {lists.map((list, idx) => (
