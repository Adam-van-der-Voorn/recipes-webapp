...
export function IngredientsSection({ ingredients }: Props) {
    ...
    return <section aria-details="recipe ingredients" className="ingredients">
        ...
        {ingredients.map((sublist, i) => {
            return <SubList key={`sublist-${i}`} sublist={sublist} renderName={hasMultipleLists} scale={scale} />;
        })}
    </section>;
}
    ...
    return <>
        ...
        <ul className="ingredientViewList">
            {sublist.ingredients.map((ingredient, i) => {
                return <IngredientRow key={`ingredient-${i}`} ingredient={ingredient} scale={scale} />;
            })}
        </ul>
    </>;
}