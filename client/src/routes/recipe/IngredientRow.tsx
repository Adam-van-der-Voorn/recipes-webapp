...
export function IngredientRow({ ingredient, scale }: Props) {
    ...
    if (typeof quantity === "string") {
        if (scale === 1) {
            ...
            const scaleStr = new Intl.NumberFormat(undefined, { maximumSignificantDigits: 3 })
                .format(scale);
            quantityStr = `x${scaleStr} ${quantity}`;
        }
    }
    else if (typeof quantity === "number") {
        quantityStr = new Intl.NumberFormat(undefined, { maximumSignificantDigits: 3 })
            .format(quantity * scale);
    }
    else {
        const valueStr = new Intl.NumberFormat(undefined, { maximumSignificantDigits: 3 })
