...
export function convertSchemaOrgRecipe(
  input: SchemaOrgRecipe,
...
function convertInstructions(
  instructionsRaw: SchemaOrgRecipe["recipeInstructions"],
): { result: string[]; warnings: Set<string> } {
  ...
  for (const el of instructionsRaw as unknown[]) {
    ...
    if (typeof el === "string") {
      ...
      const { result: subInstructions, warnings: subWarnings } =
        convertInstructions(ela["itemListElement"]);
...
type SchemaOrgIngredient =
  | SchemaOrgRecipe["ingredients"]
  | SchemaOrgRecipe["recipeIngredient"];
function convertIngredients(
  ingredientsRaw: SchemaOrgIngredient,
): { result?: Recipe["ingredients"]; warnings: string[] } {
...
function convertYield(
  input: SchemaOrgRecipe,
): { result?: string; warning?: string } {
  if (!input.recipeYield && !input.yield) {
    return {};
  }
  const yieldRaw = input.recipeYield ? input.recipeYield : input.yield;
...
function convertIso8601Duration(iso8601Duration: string): string | null {
  try {
    ...
    console.warn(
      LOG_PREFIX,
      `failed to parse ISO8601 duration ${iso8601Duration}`,
      e,
    );
    return null;
  }
