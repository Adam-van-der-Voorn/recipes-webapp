...
import {
  invalidUrl,
  unknownUnexpected,
} from "../../../../applicationErrorCodes.ts";
import { parse as parseHTML } from "node-html-parser";

const MESSAGE_BAD_URL =
  "Invalid request. The request must include a 'url' query parameter. The value of 'url' must be a valid URL.";
...
export async function extractSchemaOrg(url: URL): Promise<any[]> {
  try {
    ...
    const jsonLdList = html.querySelectorAll(
    ...
    if (jsonLdList.length === 0) {
      ...
      if (exists) {
        console.warn(
          "extract recipe: no schema.org elements found by xml parser, but evidence of recipe schema found in body via grepping",
        );
      } else {
