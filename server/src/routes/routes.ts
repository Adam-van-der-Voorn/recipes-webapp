...
export function handleRequests(
  ...
  exp.post(
    "/api/external-recipe",
    (req, res) => addRecipeFromUrl(req, res, app.firestore()),
  );
...
function serveFile(rootDir: string, filePath: string) {
  ...
  return (_req: any, res: Response) =>
    res.sendFile(filePath, opts, errCallback);
}
...
function fallThroughJson(req: Request, res: Response) {
  ...
  res.status(404)
    .json({
      ecode: notFound,
      context:
        `The requested resource does not exist, or does not support ${req.method} requests.`,
    });
}
